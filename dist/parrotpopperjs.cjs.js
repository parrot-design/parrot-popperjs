    ...
    return function createPopper(reference, popper, options = defaultOptions) {
        ...
        const instance = {
            ...
            setOptions(options) {
                ...
                state.scrollParents = {
                    reference: domUtils.isElement(reference)
                        ? domUtils.listScrollParents(reference)
                        : reference.contextElement
                            ? domUtils.listScrollParents(reference.contextElement)
                            : [],
                    popper: domUtils.listScrollParents(popper)
                };
                //根据依赖项进行排序
            ...
            forceUpdate() {
                ...
                state.rects = {
                    reference: domUtils.getCompositeRect(reference, domUtils.getOffsetParent(popper), state.options.strategy === 'fixed'),
                    popper: domUtils.getLayoutRect(popper),
                };
                state.reset = false;
...
function effect$2({ state, instance, options }) {
    ...
    const window = domUtils.getWindow(state.elements.popper);
    const scrollParents = [
...
function applyStyles({ state }) {
    Object.keys(state.elements).forEach((name) => {
        ...
        if (!domUtils.isHTMLElement(element) || !domUtils.getNodeName(element)) {
            return;
        }
...
function detectOverflow(state, options = {}) {
    ...
    const clippingClientRect = domUtils.getClippingRect(domUtils.isElement(element)
        ? element
        : element.contextElement || domUtils.getDocumentElement(state.elements.popper), boundary, rootBoundary);
    const referenceClientRect = domUtils.getBoundingClientRect(referenceElement);
    ...
    const popperClientRect = domUtils.rectToClientRect(Object.assign(Object.assign({}, popperRect), popperOffsets));
...
function preventOverflow({ state, options, name }) {
    ...
    if (checkMainAxis || checkAltAxis) {
        ...
        const arrowRect = tether && arrowElement
            ? domUtils.getLayoutRect(arrowElement)
            : { width: 0, height: 0 };
        ...
        const arrowOffsetParent = state.elements.arrow && domUtils.getOffsetParent(state.elements.arrow);
...
function arrow({ state, name, options }) {
    ...
    const arrowRect = domUtils.getLayoutRect(arrowElement);
    ...
    const arrowOffsetParent = domUtils.getOffsetParent(arrowElement);
...
function effect({ state, options }) {
    ...
    if (!domUtils.contains(state.elements.popper, arrowElement)) {
        return;
    }
...
function mapToStyles({ popper, popperRect, placement, offsets, position, gpuAcceleration, adaptive, roundOffsets, }) {
    ...
    const win = window;
    if (adaptive) {
        let offsetParent = domUtils.getOffsetParent(popper);
        let heightProp = 'clientHeight';
        let widthProp = 'clientWidth';
        if (offsetParent === domUtils.getWindow(popper)) {
            offsetParent = domUtils.getDocumentElement(popper);
            if (domUtils.getComputedStyle(offsetParent).position !== 'static') {
